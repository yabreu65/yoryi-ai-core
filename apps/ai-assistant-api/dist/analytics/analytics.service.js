...
let AnalyticsService = class AnalyticsService {
    db;
    constructor() {
        const dbPath = (0, node_path_1.join)(process.cwd(), "data", "analytics.db");
    ...
    `);
        const result = stmt.run(event.eventName, event.actionKey, event.actionLabel, event.tenantId, event.currentRoute, event.currentModule ?? null, event.targetPath ?? null, event.isMapped ? 1 : 0, event.sessionId, event.messageId, event.actionIndex, event.totalActions, event.timestamp);
        if (result.changes > 0) {
      ...
      LIMIT 1
    `)
            .get(event.tenantId, event.sessionId, event.messageId, event.actionKey, event.actionIndex);
        return existing?.id ?? 0;
    }
    trackAssistantChatMetric(metric) {
        const stmt = this.db.prepare(`
    ...
    `);
        const result = stmt.run(metric.timestamp, metric.question, metric.appId, metric.tenantId ?? null, metric.module ?? null, metric.role ?? null, metric.actionCount, metric.hasActions ? 1 : 0, metric.llmUsed ? 1 : 0, metric.knowledgeFound ? 1 : 0, metric.fallback ? 1 : 0, metric.answerSource ?? null, metric.debtQueryDetected === undefined
            ? null
            : metric.debtQueryDetected
                ? 1
                : 0, metric.debtAnswerExact === undefined ? null : metric.debtAnswerExact ? 1 : 0, metric.financialGatewayLatencyMs ?? null);
        this.trackAssistantQueryAudit(metric);
        return result.lastInsertRowid;
    }
    trackAssistantQueryAudit(metric) {
        const auditId = metric.auditId ??
    ...
    `);
        stmt.run(auditId, metric.timestamp, metric.question, metric.appId, metric.tenantId ?? null, metric.userId ?? null, metric.role ?? null, metric.route ?? null, metric.module ?? null, metric.answerSource ?? "fallback", metric.responseType ?? null, metric.dataScope ?? null, metric.llmUsed ? 1 : 0, metric.knowledgeFound ? 1 : 0, metric.debtQueryDetected === undefined
            ? null
            : metric.debtQueryDetected
                ? 1
                : 0, metric.debtAnswerExact === undefined ? null : metric.debtAnswerExact ? 1 : 0, metric.financialGatewayLatencyMs ?? null);
    }
    getDebtMetrics(tenantId, fromDate, toDate) {
      ...
      ${whereClause}
    `);
        const row = totalsStmt.get(...params);
        ...
        const exactAnswerRate = debtQueries > 0 ? Math.round((exactAnswers / debtQueries) * 10000) / 100 : 0;
        const fallbackRate = debtQueries > 0
        ...
        return {
            ...
            fallbackDebtAnswers,
            fallbackRate,
            avgGatewayLatencyMs: row?.avgGatewayLatencyMs === null || row?.avgGatewayLatencyMs === undefined
                ? null
    ...
    getEventsByTenant(tenantId, limit = 100) {
      ...
      LIMIT ?
    `);
        return this.mapRows(stmt.all(tenantId, limit));
    }
    getEventsBySession(sessionId, limit = 100) {
      ...
      LIMIT ?
    `);
        return this.mapRows(stmt.all(sessionId, limit));
    }
    mapRows(rows) {
        return rows.map((row) => ({
            id: row.id,
    ...
    getActionMetrics(tenantId, fromDate, toDate, currentModule) {
        let query = `
    ...
    getActionIndexMetrics(tenantId, fromDate, toDate) {
        let query = `
    ...
    getTenantMetrics(tenantId, fromDate, toDate) {
        let query = `
    ...
    getDailyMetrics(tenantId, fromDate, toDate) {
        let query = `
    ...
    getOverallMetrics(tenantId, fromDate, toDate, currentModule) {
    ...
    exportToCsv(tenantId, fromDate, toDate) {
        let whereClause = `WHERE 1=1`;
        const params = [];
        if (tenantId) {
            whereClause += ` AND tenantId = ?`;
        ...
        const events = this.db.prepare(`SELECT * FROM assistant_events ${whereClause} ORDER BY createdAt DESC`).all(...params);
        const headers = [
            'id',
            'eventName',
            'actionKey',
    ...
    exportSummaryToCsv(tenantId, fromDate, toDate) {
        const rows = [];
    ...
    saveFeedback(feedback) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(feedback.messageId, feedback.sessionId, feedback.tenantId, feedback.role || null, feedback.route || null, feedback.currentModule || null, feedback.rating, feedback.comment || null);
    }
    getFeedbackMetrics(tenantId, fromDate, toDate, currentModule) {
        let whereClause = `WHERE 1=1`;
        const params = [];
        if (tenantId) {
            whereClause += ` AND tenantId = ?`;
    ...
    getRecentComments(tenantId, limit = 10, currentModule) {
    ...
    getFeedbackDaily(tenantId, fromDate, toDate, currentModule) {
        let whereClause = `WHERE 1=1`;
        const params = [];
        if (tenantId) {
            whereClause += ` AND tenantId = ?`;
    ...
    getModuleMetrics(tenantId, fromDate, toDate) {
        let whereClause = `WHERE 1=1`;
        const params = [];
        if (tenantId) {
            whereClause += ` AND tenantId = ?`;
