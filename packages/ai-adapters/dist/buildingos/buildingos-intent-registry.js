...
exports.BUILDINGOS_INTENT_REGISTRY = void 0;
exports.getBuildingOSIntentDefinition = getBuildingOSIntentDefinition;
exports.getBuildingOSIntentDefinitions = getBuildingOSIntentDefinitions;
exports.resolveCanonicalIntentCode = resolveCanonicalIntentCode;
const ADMIN_ONLY_ROLES = ["SUPER_ADMIN", "TENANT_OWNER", "TENANT_ADMIN"];
exports.BUILDINGOS_INTENT_REGISTRY = [
    {
...
const intentByCode = new Map(exports.BUILDINGOS_INTENT_REGISTRY.map((definition) => [definition.code, definition]));
const aliasToCanonical = new Map(exports.BUILDINGOS_INTENT_REGISTRY.flatMap((definition) => definition.legacyAliases.map((alias) => [alias, definition.code])));
function getBuildingOSIntentDefinition(code) {
    return intentByCode.get(code);
}
function getBuildingOSIntentDefinitions() {
    return exports.BUILDINGOS_INTENT_REGISTRY;
}
function resolveCanonicalIntentCode(rawIntentCode, rawLegacyIntent) {
    const candidateCode = normalizeIntentCode(rawIntentCode);
    if (candidateCode && intentByCode.has(candidateCode)) {
    ...
    return aliasToCanonical.get(candidateAlias) ?? null;
}
function normalizeIntentCode(value) {
