...
class BuildingOSAdapter {
    ...
    async resolveDataBackedAnswer(input) {
        ...
        const intentDefinition = (0, buildingos_intent_registry_1.getBuildingOSIntentDefinition)(classification.intentCode);
    ...
    extractUnitIdFromQuestion(question) {
    ...
    buildPermissionActions(permissions) {
        const actions = [];
    ...
    filterActionsByRole(role, actions) {
    ...
    enrichActionsWithRegistry(actions) {
    ...
    executeOpenEntityAction(context, params) {
    ...
    isResidentDebtQuestion(question, context) {
    ...
    resolveModuleFromRoute(route) {
        if (route.includes("/charges") || route.includes("/finanzas"))
            return "charges";
        if (route.includes("/payments"))
            return "payments";
        if (route.includes("/units"))
            return "units";
        if (route.includes("/buildings"))
            return "buildings";
        if (route.includes("/tickets") || route.includes("/support"))
            return "tickets";
        if (route.includes("/communications") || route.includes("/comunicados") || route.includes("/avisos"))
            return "communications";
        if (route.includes("/documents") || route.includes("/documentos") || route.includes("/archivos"))
            return "documents";
        return "general";
    }
    async resolvePermissions(_userId, _tenantId, role) {
...
exports.BuildingOSAdapter = BuildingOSAdapter;
